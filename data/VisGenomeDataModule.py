...
class VisGenomeDataset(Dataset):
    ...
    def process(self,item):
        ...
        print(boxes)
        return {"img":img,"relation":torch.stack(captions),"objects":torch.stack(boxes["boxes"][:(len(boxes["boxes"])//2)]),"subjects":torch.stack(boxes["boxes"][(len(boxes["boxes"])//2):]), "obj_classes":torch.stack([self.tokenize(x) for x in obj_classes]),"subj_classes":torch.stack([self.tokenize(x) for x in subj_classes])}
